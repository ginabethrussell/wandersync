          ...
          {itineraries.map((itinerary, idx) => (
            <div key={idx} className="p-6 rounded-xl bg-white dark:bg-gray-900 shadow border border-gray-200 dark:border-gray-800">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                Destination: <strong>{itinerary.destination}</strong> | Days: {itinerary.days} | Best time: {itinerary.recommendedTime}
              </p>
              ...
              <div className="mt-4 overflow-x-auto">
                <table className="table-auto min-w-full text-sm border-collapse">
                  <thead className="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200">
                    <tr>
                      {Object.keys(itinerary.csv[0] || {}).map((key) => (
                        <th key={key} className="border px-3 py-2 text-left">{key}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {itinerary.csv.slice(0, 3).map((row, rowIndex) => (
